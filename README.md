...
| `query_vulnerabilities` | OSV.dev lookup for known vulnerabilities affecting a package, optionally scoped to a version, with `isVulnerable`/`highestSeverity` as a direct verdict and each finding's severity, summary, and fixed version. Also cross-checks the name against the npm registry (npm ecosystem only) so a typo'd/nonexistent package returns an error instead of a false "clean" `isVulnerable:false` — see `packageExists`/`existenceCheckNote`. |
| `batch_query_vulnerabilities` | OSV.dev lookup across a whole dependency inventory at once — pass a flat `packages` list, or paste raw `package.json`/lockfile/CycloneDX JSON/SPDX JSON content via `content` and it parses that for you. Chunks large inventories internally rather than stopping at OSV's own 100-package batch limit, with severity, summary, CVE aliases, and fixed version enriched per finding. For an explicit `packages` list or raw `package.json` content (names never resolved against a registry, unlike a real lockfile/SBOM), also cross-checks up to 200 unique names against the npm registry — see `unresolvedPackages`/`existenceCheckNote`; don't read a name in there as a clean bill of health. |
| `get_latest_advisories` | GitHub Security Advisories for the npm ecosystem from one of two disjoint sources (never both in one call): `type: "reviewed"` (default) is the curated, mostly CVE-backed set, filterable by severity, vulnerability category, affected package name, or an exact GHSA/CVE ID; `type: "malware"` is known-malicious packages (e.g. OpenSSF's malicious-packages feed), almost none of which carry a CVE or a meaningful CWE beyond "embedded malicious code". Cursor-paginated. |
